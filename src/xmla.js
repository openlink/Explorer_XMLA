...
(function (Date, undefined) {
    ...
    Date.prototype.toString = function() {
      var s = this.getFullYear()
        ...
        var tzInHours = -this.getTimezoneOffset()/60;
        s += (tzInHours>0)?'+':'-';
        s += pad(tzInHours)+ ':';
        var tzInMin = this.getTimezoneOffset()%60;
        tzInMin = tzInMin<0?-tzInMin:tzInMin;
        s += pad(tzInMin);
      return s;
    };
}(Date));
...
function _xmlEncodeListEntry(value){
    return value!=null?value.replace(/\&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):value;
}
...
*/
    ...
    _requestError: function(options) {
        if (options.error) options.error.call(options.scope ? options.scope : null, this, options, options.exception);
        if (options.callback) options.callback.call(options.scope ? options.scope : null, Xmla.EVENT_ERROR, this, options, options.exception);
